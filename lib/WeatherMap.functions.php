...
function format_number_weathermap($number, $precision = 2, $trailing_zeroes = 0)
{
	$sign=1;
...
function nice_bandwidth($number, $kilo = 1000,$decimals=1,$below_one=TRUE)
{
	...
	$result=format_number_weathermap($number, $decimals) . $suffix;
	return ($result);
}
...
function nice_scalar($number, $kilo = 1000, $decimals=1)
{
	...
	$result = $prefix . format_number_weathermap($number, $decimals) . $suffix;
	return ($result);
}
